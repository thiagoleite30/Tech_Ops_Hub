...
class RequestForms(forms.ModelForm):
    ...
    class Meta:
        ...
        exclude = ['chamado', 'concluida', 'usuario',
                   'data_conclusao', 'existe_novo_pdv']
        labels = {
            'gpos': 'GPOS',
            'pdv_atual': 'PDV Atual',
            'loja_nova': 'Nova Loja',
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields['gpos'].queryset = GPOS.objects.filter(blocked=False,
                                                           ...
                                                           last_logon_date__gte=periodo
                                                               ...
                                                               'ativo', 'loja', 'pdv').order_by('pos_number').distinct('pos_number')
        localizacoes = Location.objects.prefetch_related('local_pai').all()

        self.fields['loja_nova'].queryset = localizacoes.filter(
            sub_locations__isnull=False).distinct()
